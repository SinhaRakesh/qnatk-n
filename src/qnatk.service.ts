...
export class QnatkService {
  ...
  constructor(
    ...
    @Inject('CONNECTION_NAME') private connectionName: string,
    @Inject('MODEL_ACTIONS')
    private modelActions: Record<string, ActionListDTO>[] = [],
  ) {}

  onModuleInit() {
    const token = getConnectionToken(this.connectionName) as string;
    console.log('qnatk service', token, this.connectionName);
    this.sequelize = this.moduleRef.get<Sequelize>(token, { strict: false });
  }
